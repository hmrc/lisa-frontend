  ...
  val form: Form[OrganisationDetails] = Form(
    mapping(
      "companyName" -> optional(text)
        ...
        .verifying(
          "error.companyNamePattern",
          i => i.isEmpty || i.getOrElse("").length > 65 || i.getOrElse("").matches(COMPANY_NAME_REGEX)
        ),
      "ctrNumber"   -> optional(text)
        ...
        .verifying(
          "error.ctrNumberPattern",
          i => i.isEmpty || i.getOrElse("").length != 10 || i.getOrElse("").matches(UTR_REGEX)
        )
    )((name, utr) =>
      OrganisationDetails(
        name.getOrElse(""),
