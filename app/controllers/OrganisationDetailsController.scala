            ...
            .fold(
              ...
              data =>
                sessionCacheRepository
                  .putSession[OrganisationDetails](DataKey(OrganisationDetails.cacheKey), data)
                  .flatMap { _ =>
                    logger.info(
                      s"[OrganisationDetailsController][Post] BusinessStructure retrieved: ${businessStructure.businessStructure}"
                    )
                    rosmService.rosmRegister(businessStructure, data).flatMap {
                      case Right(safeId: String) =>
                        auditService.audit(
                          auditType = "BusinessStructureSuccess",
                      ...
                      case Left(error)           =>
                        ...
                        auditService.audit(
                          auditType = "BusinessStructureFailed",
                          path = routes.OrganisationDetailsController.post.url,
